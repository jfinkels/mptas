...
\begin{definition}\label{def:ptasreduction}
  ...
  There is an \emph{MPTAS reduction} from $P$ to $Q$ if there are functions $f \colon \left(I_P \times \mathbb{Q}^+\right) \to I_Q$, $g \colon \left(I_P \times \Sigma^* \right) \to \Sigma^*$, and $c \colon (0, 1) \to (0, 1)$ such that for all $x \in I_P$ and all $\epsilon \in (0, 1)$,
  \begin{enumerate}
...
In the last steps of the above proof we ignore strings $x$ that were not ``sufficiently long''.
This is acceptable because a lookup table of finite size can store the optimal solutions to all instances of optimization problem $P$ of length up to $N$, where $N$ is a constant (representing the ``sufficient length'').
